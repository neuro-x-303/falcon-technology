...
if (canvas) {
    ...
    function triggerPulse(start, end) {
        if (Math.random() > 0.98) {
            ...
            activePulses.push({ mesh: pulse, start: start.clone(), end: end.clone(), progress: 0 });
    ...
    function animate() {
        ...
        for (let i = 0; i < nodes.length; i++) {
            for (let j = i + 1; j < nodes.length; j++) {
                ...
                if (dist < maxDistance) {
                    linePositions.push(nodes[i].pos.x, nodes[i].pos.y, nodes[i].pos.z, nodes[j].pos.x, nodes[j].pos.y, nodes[j].pos.z);
        ...
        for (let i = activePulses.length - 1; i >= 0; i--) {
            ...
            p.progress += 0.02;
            if (p.progress >= 1) { pulseGroup.remove(p.mesh); activePulses.splice(i, 1); }
...
if (typingText) {
    const phrases = ["TECH APPLICATIONS", "AI POWERED SYSTEMS", "MANAGEMENT TOOLS", "ENTERPRISE SOLUTIONS", "DREAM ARCHITECTURES"];
    ...
    function type() {
        ...
        if (!isDeleting && charIndex === currentPhrase.length) { isDeleting = true; typeSpeed = 1500; }
