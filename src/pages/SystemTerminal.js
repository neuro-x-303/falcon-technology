  ...
  return (
    <main className="dashboard">
      ...
      <section className="projects-section-container" style={{ position: 'relative', zIndex: 1 }}>
        ...
        <div className="dashboard-content-layout" style={{ display: 'grid', gridTemplateColumns: '1fr 350px', gap: '2rem' }}>
          ...
          <aside className="activity-feed glass-container" style={{ padding: '1.5rem', background: 'rgba(2, 6, 23, 0.4)' }}>
            <h3 style={{ fontSize: '0.9rem', fontFamily: 'Orbitron', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <Activity size={16} color="var(--primary-color)" /> NEURAL ACTIVITY
            </h3>
            <div className="feed-items" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {[
                { time: '0.02ms ago', msg: 'Neural packet uplink from Node-Alpha verified.', type: 'success' },
                { time: '1.4s ago', msg: 'Gamma node throughput exceeded 12.1k Hz.', type: 'info' },
                { time: '5.2s ago', msg: 'Security matrix Level 9 synchronization complete.', type: 'success' },
                { time: '12s ago', msg: 'API Layer-Sigma latency spike detected (0.92ms).', type: 'warning' },
                { time: '1m ago', msg: 'System heartbeat normal across all meshes.', type: 'info' }
              ].map((item, i) => (
                <div key={i} style={{ borderLeft: `2px solid ${item.type === 'success' ? 'var(--success)' : item.type === 'warning' ? 'var(--accent-color)' : 'var(--primary-color)'}`, paddingLeft: '1rem' }}>
                  <p style={{ fontSize: '0.85rem', marginBottom: '0.2rem' }}>{item.msg}</p>
                  <span style={{ fontSize: '0.7rem', color: 'var(--text-secondary)', fontFamily: 'monospace' }}>[{item.time}]</span>
                </div>
              ))}
            </div>
          </aside>
